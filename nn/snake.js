class Snake {
  ...
  createInputs() {
    ...
    const input2 = {food_left: 0, food_right: 0, food_top: 0, food_bottom: 0, wall_left: 0, wall_right: 0, wall_top: 0, wall_bottom: 0, closest_food_angle: 0};
    ...
    input2.closest_food_angle = inputs[8];
  ...
  createOutputs() {
    ...
    /*
    return [
      this.moveX == -1 ? 1.0 : 0.0, //left
      this.moveX == 1 ? 1.0 : 0.0, //right
      this.moveY == -1 ? 1.0 : 0.0, //up
      this.moveY == 1 ? 1.0 : 0.0, //down
    ];
    */
  }
  ...
  getAngle() {
    ...
    // 1: in front, 0: behind
    let res = 1 - abs(degrees(vectToFood.angleBetween(dir)) / 180);
    if (isNaN(res)) {
      // snake is on food
      return 0;
    }
